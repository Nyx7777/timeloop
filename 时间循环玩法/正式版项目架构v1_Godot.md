...
- 界面端到端冒烟测试通过 12 项检查，覆盖 T1 移动/攻击/死亡、时间线转换、T2 分身重演和胜利提示；核心无画面测试仍通过 71 项检查。
- M1 剩余工作是用户实际试玩验收，以及为完整行动记录补齐独立保存/重新加载入口；验收通过后进入 M2 第 5 关架构压测。
