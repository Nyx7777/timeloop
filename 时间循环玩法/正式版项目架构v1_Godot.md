...
- M5.0B：完善逐敌人的固定历史意图、等待、序列映射，以及“扰动但本回合仍锁定 → 下一回合清醒 → 实时 AI”的状态表达。
...
- 扰动状态按单个敌人、单条时间线存储；当前锁定意图不改变，从下一回合起重新计算 AI，并将实际行为写入当前时间线的新历史。切换时间线时清除清醒状态，新行为作为固定历史重演；新分身再次造成有效位移时按一次新扰动处理。
