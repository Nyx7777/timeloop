...
- 将逻辑格坐标映射到当前近俯视 2D 棋盘坐标；投影策略可在表现层独立替换。
...
- 规则内核只认逻辑格；近俯视投影与触控命中映射由表现层的棋盘映射器单独负责。
...
在正式绘制大量素材前，先完成一个“美术基准格”：确定近俯视方格比例、角色脚底锚点、角色最大外扩范围、墙体侧面高度、阴影方式和像素缩放策略。基准格未确认前，不批量生产角色和地块。
...
- 击退、空洞死亡和位移失败。
- 历史移动撞击。
- 扰动在下一回合清醒；分身重演完成后必须先通过历史一致性判断，不能把匹配历史的位移误判为扰动。
- 死亡、固化、时间线重置和最后一命失败。
...
- M4.1：在不修改规则内核、敌人 AI 和关卡数据的前提下，把战斗主场景切换为 390×844 竖屏逻辑基准。
- 建立顶部 HUD、行动序列、近俯视完整 8×8 棋盘、固定四按钮和触控点格输入。
- 用 360×800、390×844、430×932 三种竖屏比例验证容器布局、触控尺寸和无裁切。
- M4.2：接入第一批正式角色、地砖、UI 组件与动画状态机；继续由 `BattleEvent` 驱动表现。

### M5：对外体验 Demo

- M5.0A：建立战斗内外共享 UI 设计系统，正式化顶部栏、行动序列、情境提示和底部按钮；先完成四张代表状态稿与三种竖屏比例评审，再批量接入。
- M5.0B：完善逐敌人的固定历史意图、等待、序列映射，以及“分身重演后检查历史一致性 → 偏离才扰动但本回合仍锁定 → 下一回合清醒 → 实时 AI”的状态表达。
- M5.0C：接入现有六关并执行无日志可读性、复杂状态和三种手机比例回归；M4 的规则、动画与布局基线不得回退。
- M5.1：设计标题、准备、路线、节点、战后、奖励、事件、Boss 和本轮总结组成的战斗外 UX 与线框。
- M5.2：建立 `RunSession`、路线图、节点状态、Build/资源、坍缩、待领取奖励与序列化骨架。
- M5.3：打通路线选择、节点进入、战斗、结算、奖励、返回地图、事件/资源和 Boss 的最小宏观闭环。
- M5.4：制作推荐约 6 节点、20—30 分钟的首个对外 Demo；在进入内容生产前裁决第二种武器、监视、坍缩实际作用和合并回放深度。
- M5.5：进行从标题页开始的无讲解外部测试，并按高频理解问题与阻断问题收敛版本。
- 完整页面、交付物、验收门禁与延期范围统一见 `M5对外Demo与战斗内外UI规划.md`。完整商店、动态追兵、Meta 与多章节不默认进入首版。
...
- M1 尚未完成的工程项是为完整行动记录补齐独立保存/重新加载入口；表现修正与 M2 一并交由用户验收。
- 重开后的预览残留已修复：`sync_from_state()` 作为表现层完整同步入口，会清空旧回合的分身火线、敌人意图、击退预览、脉冲和悬停缓存。
- M2 第 5 关“历史冲撞”已完成正式资源迁移与可见接入，主场景支持第 1/5 关切换并默认进入第 5 关。
- `BattleState.rules` 保存数据驱动的关卡规则开关；击退和固化能力随战斗状态、检查点与序列化共同恢复。
- 普通攻击和分身攻击共用 `DisplacementQuery`：保存固定方向，统一判断移动、受阻或坠入时间空洞，表现层同时使用该查询生成无副作用的落点预览。
- 固定敌人历史移动拥有终点优先权；玩家占据终点时按最后移动方向被撞开，进入空洞或没有合法退路时死亡。
- 扰动状态按单个敌人、单条时间线存储；当前锁定意图不改变，从下一回合起重新计算 AI，并将实际行为写入当前时间线的新历史。切换时间线时清除清醒状态，新行为作为固定历史重演。分身位移不即时扰动；全部分身完成后，将每名固定敌人的实际位置与锁定意图的 `from` 起点比较，匹配则是正常历史重建，偏离或缺失固定意图才触发新扰动。本体阶段的新有效位移仍即时扰动。
- 主动固化作为提交边界：仅在关卡解锁、玩家输入阶段且非最后一命时可用，立即提交录像、消耗命数并跳过敌方阶段。
- M2 核心无画面测试通过 117 项，界面端到端测试通过 24 项；真实渲染检查覆盖第 5 关初始布局与击退落点。
- 当前等待用户合并验收 M1.5 修复和 M2 第 5 关；通过后进入 M3 六关迁移与视觉基准。
